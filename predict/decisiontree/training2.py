...
class TrainingSet(object):
    ...
    def split(self, dimension, split_value):
        ...
        index = self._index[dimension]
        null_table._items = [entry for entry in self._get_items() if entry[index] is None]
        left_table._items = [entry for entry in self._get_items() if entry[index] <= split_value]
        right_table._items = [entry for entry in self._get_items() if entry[index] > split_value]
